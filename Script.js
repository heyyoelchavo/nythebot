...
<div class="ui inverted segment">
  <div class="ui inverted form">
	...
	<div class="field" id="botname">
        <label>Name</label>
        <div class="ui large labeled input"><input type="text" value="${params.get('name') || "AdolfXD"}" placeholder="Nickname" maxlength="17" spellcheck="false" data-ms-editor="true"></div>
      </div>
      <div class="field">
        <label>Room</label>
      ...
      <div class="field" id="botamount">
        ...
        <div class="ui large labeled input"><input type="number" value="${params.get('amount') || "31"}" min="1" max="20"></div>
      </div>
    </div>
      ...
      try {
        ...
        if (people) {
          ...
          if (regex.test(username)) {
            document.querySelector('#botname div input').value = "AdolfXD";
            username = "";
          }
        }
    ...
    socket.addEventListener('message', (event) => {
      ...
      switch (data[0]) {
        case 5: {
          ...
          if (params.get('private-mode') !== "true") { socket.send(`42[11,"${playerId}"," Adolf Airports iyi uçuşlar diler"]`); }
...
document.querySelector("#send").addEventListener("click", function () {
  ...
  if (regex.test(messagetext)) {
    document.querySelector('#messagetext input').value = "AdolfXD";
    messagetext = "AdolfXD";
  }
    ...
    socketList.forEach((socket) => {
      if (socket.readyState === WebSocket.OPEN) {
        ...
        if (regex.test(spamtext)) {
          document.querySelector('#spamtext input').value = "sen benim dengim değilsin";
          spamtext = "sen benim dengim değilsin";
        }
        const randomIndex = Math.floor(Math.random() * (spamtext.length + 1));
        let modifiedMessage;

        if (Math.random() < 0.1) {
          modifiedMessage = "ez win haha";
        } else {
